"""带代理与多源接力的 HTTP 客户端（移植自 SUG updater/http_client.py）。

封装 ``requests``：

* 透传 ``proxies`` 字典与可选 ``session``（KS 的 ``requests_session_for_proxy``
  会在代理模式为 off/manual 时关闭 ``trust_env``，必须沿用同一个 session）；
* 提供 ``get_json`` / ``get_text`` / ``get_redirect_location`` / ``download``；
* :class:`SourceTrialRunner` 把"多个候选 URL 按顺序尝试"收敛成一个入口。
"""
...
def _headers() -> Dict[str, str]:
    return {
        "User-Agent": f"KaraokeStudio-Updater/{APP_VERSION} (+https://github.com/karaoke-studio/karaoke-studio)",
...
class HttpResult:
    ...
    status: int = 0
    error: str = ""
    # 仅 ``get_json`` / ``get_text`` / ``get_redirect_location`` 使用
...
def get_redirect_location(
    ...
    *,
    ...
    ``/releases/tag/<tag>``。该网页端点不计入 ``api.github.com`` 的
    "未认证 60 次/小时（按出口 IP）"限流，因此可在 API 全部返回 403 时
    作为轻量兜底，仅用来探测最新版本号。
            ...
            with http.get(
                ...
                if existing_size > 0 and resp.status_code == 200:
                    ...
                    if attempt < DOWNLOAD_RETRY_COUNT:
                        ...
                        continue
                    return HttpResult(ok=False, status=resp.status_code, error=last_error)
...
class SourceTrialRunner:
    ...
    def run(self, op: Callable[[str], HttpResult]) -> Optional[HttpResult]:
        ...
        所有候选均失败返回 ``None``，可读 :attr:`attempts` 获取各源失败原因。
        """
    ...
    def summary(self) -> str:
        ...
        for a in self.attempts:
            parts.append(
                f"[{a.source_id}] {'OK' if a.ok else 'FAIL'}" + (f" {a.error}" if a.error else "")
            )
        return " | ".join(parts)
