...
class PropertyPanel(QWidget):
    ...
    def _make_vertical_layout_section(self) -> QFrame:
        ...
        self._allow_inter_page_line_overlap_check.setToolTip(
            "系统会自动缩短发生冲突的提前入场和延迟退场时间，但不会截断任何"
            "走字区间。自动压缩可以缩短动画时段：不会把非零入场动画自动压到"
            " 250 ms 以下；若动画时长或上屏时间由用户手工设定，则保留用户值；"
            "退场动画可以缩短为 0 ms。关闭时，还会根据字幕"
            "正文、注音、描边、阴影和发光在布局"
            "完成后的绘制范围，移动后进入的整页字幕。避让优先沿布局方向寻找画布"
            "内空间，跨页空隙采用被重叠页面布局的行间距；放不下时改向反方向寻找；"
            "两边都放不下则保持原布局位置和绘制优先级。页面一旦移动，会保持位置"
            "直到本页播放完毕。前后页面排版一致时，入场、退场和字符动画允许"
            "互相穿越；排版变化时，新页面的入场动画也参与避让，上一页退场动画"
            "仍允许重叠。开启后不执行"
            "空间避让，允许跨页字幕"
