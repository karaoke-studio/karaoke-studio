...
- **视频成片只输出 MP4、只支持 60/120fps**；另支持两种附加导出：**PNG 序列**
  ...
  `名称_000001.png`，重名非空目录前置拦截）与**透明字幕视频 QuickTime 动画**
  （`.mov`，qtrle 无损 alpha，仅字幕层不带背景，2026-09 新增）。仍不做 30fps
  原样输出、AVI 或 N3 ARGB 帧序列；PNG/mov 格式的完成弹窗不提供「进入下一步」
  （第 6 步混流只收 MP4）。输出格式偏好持久化在 app-local `output` 命名空间。
