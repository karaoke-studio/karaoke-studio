...
- **主程序 EXE 双名并存**：包里主程序是 `Lin-K Lyrics.exe`，同时必须带一份同内容的`Karaoke Studio.exe`，并且它要在 `build_parts.APP_TARGETS` 里。执行更新的是旧版代码，它按旧文件名校验更新包并重启 —— 删掉副本会让存量用户全量路径彻底断更、增量路径「装上了却拉不起来」。原理与失败矩阵见 [docs/auto_update.md §8.1](docs/auto_update.md)，护栏见 `tests/test_rename_release_invariants.py`。**2026-09 迁移机制**（为将来停发旧名副本做准备，见 §8.1 末尾的收尾清单）：新版主程序 `installer.find_app_exe_name()` 固定返回新名（与启动文件名无关），Updater 在**新版本成功拉起之后**由 `_cleanup_legacy_main_exe` 迁移旧名快捷方式（`.lnk` 原地改指新名，尽力而为）并清理本地旧名副本——迁移失败不阻断清理（宁可留坏快捷方式也不留旧名副本），仅新名入口缺失时跳过清理；迁移期打包与 `APP_TARGETS` 保持双名不动。
...
- **视频成片只输出 MP4、只支持 60/120fps**；另支持附加导出：**PNG 序列**
  （透明字幕层 / 含背景合成，输出到以导出名命名的独立子文件夹，帧名
  `名称_000001.png`，重名非空目录前置拦截，无音频）与**两种透明字幕视频**
  （`.mov`，仅字幕层不带背景，携带与 MP4 同源的音频 pcm_s16le，2026-09 新增）：
  **ProRes 4444**（`prores_ks` + `yuva444p10le`，NLE alpha 兼容性最好，默认推荐）
  与 **QuickTime 动画**（`qtrle` + `rgba`，无损、体积小；多数播放器不合成其
  straight alpha 而显示黑底——这是播放器行为不是导出错误，导入 PR/AE 即正常，
  不要再因「播放黑底」误判为编码缺陷而下线）。仍不做 30fps 原样输出、AVI 或
  N3 ARGB 帧序列；PNG/mov 格式的完成弹窗不提供「进入下一步」（第 6 步混流
  只收 MP4）。输出格式偏好持久化在 app-local `output` 命名空间。
