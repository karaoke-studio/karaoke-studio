...
@pytest.mark.parametrize(
    "name",
    [
        "_title_duration_spin",
        "_title_head_spin",
        "_title_tail_spin",
        "_title_tail_duration_spin",
    ],
)
def test_every_title_time_field_accepts_the_full_range(panel, name: str) -> None:
    spin = getattr(panel, name)

    assert spin._maximum == TITLE_TIME_MAX_MS
    # 秒位得跟着放开，否则界面上打不到上限。
    assert spin.seconds_spin.maximum() == TITLE_TIME_MAX_MS // 1000


def test_the_duration_reaches_the_model_unclamped(panel) -> None:
    panel._title_duration_spin.setValue(TITLE_TIME_MAX_MS)

    assert panel._title_duration_spin.value() == TITLE_TIME_MAX_MS
...
def test_a_value_past_the_old_cap_survives(panel) -> None:
    ...
    panel._title_duration_spin.setValue(1_200_000)

    assert panel.subtitle_style.title_overlay.duration_ms == 1_200_000


@pytest.mark.parametrize("name", ["_title_fade_in_spin", "_title_fade_out_spin"])
