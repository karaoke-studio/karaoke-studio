...
# ---------------------------------------------------------------------------
# P1.c-1：ユートピア bake+变换（opt-in，默认关）
# ---------------------------------------------------------------------------


def _utopia_style(**kw):
    return Style(
        line_y_position="center",
        entry_anim="utopia",
        exit_anim="utopia",
...
def test_utopia_bake_opt_in_renders_close_to_vector(qapp, monkeypatch):
    # 开关开启后 utopia 走 bake+变换，应与逐帧光栅化几何一致（仅边缘 AA 微差）。
    track = _track()
    t = 1600  # 入场/扫光阶段，画面有墨迹
    monkeypatch.delenv("KROK_SUBTITLE_UTOPIA_BAKE", raising=False)
    clear_before_layer_cache()
    vec = _blank()
    paint_frame(vec, track, t, _utopia_style())

    monkeypatch.setenv("KROK_SUBTITLE_UTOPIA_BAKE", "1")
    clear_before_layer_cache()
    bake = _blank()
    paint_frame(bake, track, t, _utopia_style())
    clear_before_layer_cache()

    assert _pixel_hash(vec) != _pixel_hash(_blank())  # 逐帧光栅化非空
    assert _pixel_hash(bake) != _pixel_hash(_blank())  # bake 非空
    # 墨迹包围盒接近（几何一致，超采样后仅 AA 微差）
    for a, b in zip(_ink_bounds(vec), _ink_bounds(bake)):
        assert abs(a - b) <= 5


def test_utopia_bake_flown_out_stays_blank(qapp, monkeypatch):
    # 退场飞出后仍应为空（bake 路径不得让已飞出的字重现）。
    track = _track()
    monkeypatch.setenv("KROK_SUBTITLE_UTOPIA_BAKE", "1")
    clear_before_layer_cache()
    utopia = _blank()
    paint_frame(
        ...
        track,
        3600,
        Style(line_y_position="center", line_tail_ms=1100, exit_fade_ms=1000, exit_anim="utopia"),
    )
    clear_before_layer_cache()
    assert _pixel_hash(utopia) == _pixel_hash(_blank())


def test_utopia_bake_populates_and_clears_cache(qapp, monkeypatch):
    from krok_helper.subtitle_render.engine.painter import _UTOPIA_BAKE_CACHE
...
def test_utopia_bake_default_off_unchanged(qapp, monkeypatch):
    # 不设开关时必须走原逐帧光栅化路径（像素与历史一致，缓存不被填充）。
    from krok_helper.subtitle_render.engine.painter import _UTOPIA_BAKE_CACHE
