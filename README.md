...
界面以一条「6 步工作流」串联从素材下载、歌词制作到成品混流的完整链路。
...
- `歌词检索`：多来源搜索、预览、复制歌词
- `歌词打轴`：嵌入 StrangeUtaGame，完成逐句 / 逐字时间轴与注音处理
- `字幕视频生成`：预览、编辑并渲染卡拉 OK 动画字幕 MP4
...
- 嵌入 [StrangeUtaGame](https://github.com/karaoke-studio/StrangeUtaGame) 完整打轴界面
- 支持逐句 / 逐字时间轴、注音、角色与演唱者标签
- `.sug` 项目可直接传递到下一步字幕视频生成

### 5. 字幕视频生成

- 导入 `.sug` / `.lrc` / `.n3proj`，支持多字幕源和逐行角色 / 特效
- 视频背景预览，播放、拖动和字幕时间轴同步
- 支持 ruby、渐变、图片填充、描边、发光、竖排、RTL、标题和动画
- 使用 QPainter + FFmpeg 多进程渲染，输出 60 / 120 fps MP4
- 导出结果可直接传递到第 6 步 Hi-Res 混流
