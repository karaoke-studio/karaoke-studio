...
2. **多 checkpoint 主文字不同**：SUG 即使没有 ruby 也能用第二、第三 checkpoint 改变主文字速度；字幕 LRC 正文只保留每字第一个 checkpoint，没有 ruby 时其余 checkpoint 会丢失。
3. **ruby 的空间权重不同**：SUG 通常按 ruby part 实际显示宽度推进；字幕模块把每个 reading unit 对总进度的贡献视为等份。
4. **行首/行尾缺锚点不同**：SUG 会跨行借时间并尊重“未完整打轴屏障”；字幕模块基本按行解析，行尾无结束点时固定回退 500 ms。
...
- **有 ruby**：字幕模块可能通过 `RubyAnnotation` 重新获得多段节奏；
...
- 这些字符统一使用第一个时间戳作为 `start_ms`；
- 相邻字符起点相同，前几个区间为零时长，会在越过该时刻后瞬间完成；
- 不向上一行借时间。
...
- `TimingLine.end_ms is None` 时，末字符固定用 `start_ms + 500`；
- 不跨行借下一行首；
- 行级显示结束另有 `last.start_ms + 1000` 的窗口兜底，但字符走字本身使用 500 ms。

这是一个非常明确的差异：没有显式行末释放点时，两边尾字完成时刻通常不同。
...
Nicokara LRC 不保留完整的 `check_count` 与“应该有但尚未打”的状态。字幕模块只能看到已有文本/时间戳，因此没有等价屏障，往往会按相邻时间或 500 ms 回退继续生成走字。
...
没有 ruby 的字符只有 `(start_ms, end_ms)`，整字线性。

有 `RubyAnnotation` 时，`_karaoke_fill_segments()` 会把 ruby 命中的全部主文字索引合为一个 segment，并使用 `_ruby_progress_ratio()` 驱动整段主文字锋面。

因此字幕模块不是“所有多 checkpoint 主文字都支持”，而是“**能被 ruby 映射覆盖的主文字组**可以继承 ruby 节奏”。
...
字幕模块先把 `reading` 拆成 reading unit：

- 小假名、长音符、组合浊点等按 `_RUBY_COMBINING_CHARS` 规则并入前一单元；
- `reading_part_ms` 被解释为相对 `pos_start_ms` 的边界；
- 边界不足时，剩余时长自动等分补齐；
- 若时间戳数量足够像“release/next start”成对出现，则解释为带停顿的交替区间。

`_ruby_progress_ratio()` 最终使用：

```text
(已完成 unit 数 + 当前 unit 局部比例) / unit 总数
```

也就是说每个 reading unit 对总扫光宽度贡献相同，不按实际字形宽度加权。
...
- 字幕模块：两个 reading unit 各推动 50%。

两边时间边界可能一致，但相同时间点的锋面横向位置不同。

### 11.5 暂停表达也不同

SUG group ruby 通过分段累计宽度自然表达“某段消耗时间但不推进墨水”。

字幕模块会在 `reading_part_ms` 足够多时，把它解释为 `(当前 unit 释放, 下一 unit 开始)` 成对时间；两者都能形成停顿，但判断条件和数据来源不同，异常/不完整的 timestamp 序列会走不同回退。
...
- 字幕模块：两个 reading unit 各占 50% 总进度。
...
- 字幕模块：尾字从自身起点起 500 ms 走完。

### 18.5 行首有无时间戳连读字

- SUG：可能从上一行末借时间平滑进入首 leader；
- 字幕模块：这些字与首时间戳同起，零时长字符会瞬间完成。
...
| 行首无锚点 | 跨行借时间 | 同首时间戳/零时长 | 是 |
| 行尾无结束点 | 下一行首/音频结束 | `start+500ms` | 是 |
| 未完成屏障 | 有 | 无等价信息 | 是 |
| 无 ruby 多 checkpoint | 保留 | 丢失 | 是 |
| 有 ruby 主文字组 | 特定条件合并 | ruby 命中即合并 segment | 是 |
| ruby 空间权重 | part 像素宽度 | reading unit 等份 | 是 |
...
2. **决定无 ruby 多 checkpoint 的传递方式**：现有 Nicokara 正文无法携带全部点。需要扩展 IR/WorkflowContext 直传，或定义工作台私有元数据；仅改 Painter 无解。
3. **统一行尾无结束点策略**：选择 SUG 的“下一行首/全曲结束”，或保留固定 500 ms，但必须明确产品规则。
4. **统一行首无时间戳字符策略**：决定是否跨行借锚点，以及未完成数据在最终渲染里应阻断还是容错合成。

### P1：统一正常连读和 ruby 速度

1. ~~把多字共享时间块延迟到布局阶段，按主文字 advance 分配。~~ **已完成（Python 横排）**；
2. 决定是否继续复刻 SUG 中 ruby/marker 对字符时间权重的额外撑宽；
3. 让 `_ruby_progress_ratio()` 可选按 reading unit 像素宽度累计，而非固定等份；
