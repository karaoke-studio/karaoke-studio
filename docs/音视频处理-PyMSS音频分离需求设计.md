...
§8.6 的自动导入只认「按 catalog 结构摆进 `models/`」的模型，MSST 扫描又依赖
MSST-WebUI 自己的 map 文件。二者都覆盖不到「手上就有一个 `.ckpt`，放在任意目录」
的情况，因此设置页「模型与输出轨」的每个任务都提供**「从文件导入…」**。

流程：选权重 → 自动识别同目录同名 `.yaml` → 读出真实输出轨 → 选架构 → 导入并绑定。

约束与理由：

- **原文件原地保留**：只在工作台自己的用户模型清单里登记引用，不复制、不移动、
  不修改（与 MSST 映射同一条约束，§4.5）。
- **配置只认同名文件**：找不到同名 `.yaml` / `.yml` 就留空让用户手动指定，不做模糊
  匹配——把不相干的配置套到模型上比报错更糟。
- **架构必须由用户选**：`model_type` 决定 PyMSS 用哪个加载器，而它**不在配置文件里**
  （MSST 是从自己的 map JSON 拿的），无法推断。因此给出 PyMSS 支持的 16 种架构下拉，
  仍然是选而不是填。选错不会静默出错：注册后仍要走真实加载验证，失败会标为不兼容。
- **读不出输出轨就不放行**：需要配置的架构若解析不出 `training.instruments`，
  禁用导入按钮并说明原因——不确定模型产出什么就不该允许绑定。

候选构造复用 `msst.build_candidate`（原 `_candidate` 参数化后公开）：状态判定、大小与
哈希必须与 MSST 扫描完全一致，否则同一个文件经不同入口导入会得到不同结论。
`candidate_id` 以来源前缀区分（`msst:` / `local:`），注册表据此记录 `source`。

> 合并记录：`msst._config_instruments` 原本自带一份 `training.instruments` 解析，
> 但它要求序列项比键缩进更深，读不出「序列项与键同缩进」的写法——而 PyMSS catalog
> 下发的配置正是这种（真实的 `inst_v1e.yaml` 就读不出来）。现已统一委托给
> `stems.parse_model_stems`，两种写法都支持，顺带修掉这个潜伏缺陷。

### 8.8 一键导入模型文件夹

§8.7 一次只导一个文件，逐个指定架构。当用户手上已经有一整个模型目录时（PyMSS 的
`models/`、MSST 的 `pretrain/`），设置页「模型与输出轨」顶部的**「一键导入模型文件夹」**
可以选一个目录，自动为三个任务各匹配一个模型。
...
匹配规则：只考虑 `bindable` 的候选；与推荐预设**同名的优先**，其余按名字排序保证结果
...
三张任务卡由「点一下立刻执行单个」改为**可勾选**，下方一条统一操作栏
「开始分离（已选 N 项）」按 人声 → 伴奏 → 和声伴奏 的固定顺序依次执行。

- **卡片是选择区**：整卡可点即切换勾选，选中态用主色描边 + 复选框；不可选时给出
  中文原因而非仅置灰（§3.4-3）。
- **下载确认合并到一次**：勾选项里若有缺模型的任务，操作栏汇总显示需下载体积，
  开始时只确认一次，而不是每张卡各弹一次。
- **执行中锁定**：正在跑的卡显示「进行中」，队列中的显示「排队中 · 第 N 位」，
  全部不可改选；当前任务区标题带「第 n / 共 m 个」。
- **单个失败继续跑剩余**：三个任务相互独立，一个失败不应阻塞其他。失败项作为一条
  结果记入结果区（红色 ✗ + 原因），而不是把整页打成错误态。

`request_tasks` 负责排队，`request_task` 保留为「一项的批次」，两者共用同一套记账，
界面统一显示 (n / 共 m 个)。

**音频素材不再跨会话保留**：它是「这一首歌」的临时选择而非偏好，重开软件应当是空的；
输出目录与输出格式仍然持久化。

### 8.10 直接驱动已有 MSST 环境

MSST-WebUI 是 PyMSS 的前身，很多用户手上已经有一套能跑的环境。首次配置页新增第四个
入口**「使用已有 MSST」**：工作台直接用它跑分离，用户不必再下一份 2.5 GB 的 PyMSS
托管 Runtime。

**不启动它的 WebUI**。MSST 自带命令行与完整运行环境（`workenv/python.exe` 内含
torch），我们用它的解释器执行**工作台自己的桥接脚本**。

**为什么是常驻桥接进程而不是调 `scripts/msst_cli.py`**——实测数据：

| | 耗时 |
|---|---:|
| 第 1 个任务（含加载模型） | 15.2 s |
| 第 2 个任务（同模型，命中缓存） | 1.4 s |

队列里「分离人声」和「分离伴奏」共用 `inst_v1e`，走 CLI 每个任务都要重付约 14 秒。
常驻进程还有两个好处：`MSSeparator.store_dirs` 支持按 instrument 传字典，可以**只落
我们要的那一条轨**（CLI 只能把所有 stem 写到同一个目录）；以及能回传阶段。

代价是耦合 MSST 内部 API（`inference.msst_infer.MSSeparator`），因此连接时做**真实
导入探测**，让版本漂移在配置阶段就暴露，而不是任务跑到一半才炸。

**边界与差异**（`MsstSeparationBackend` 是 `SeparationBackend` 的第三个实现，与
PyMSS 后端并列，不是它的分支）：

- **不修改用户目录**（§4.4）：桥接脚本与日志一律写在工作台自己的工作目录；
- **没有模型下载**：MSST 用自己的 DownloadManager，工作台只使用目录里已有的模型，
  任务卡相应显示「未指定模型 / 模型文件缺失」而不是「需下载 x.xx GB」；
- **没有 Runtime 安装与校验**：环境是用户的，只读探测；
- **不使用 PyMSS 的用户模型注册表**：模型路径直接交给 `MSSeparator`，绑定信息存在
  `settings.pymss["msst_bindings"]`；
- **取消任务需重启桥接进程**：一次推理不可中断，与托管 PyMSS 的处理方式一致（§9.3）。

首次连接会按推荐模型自动映射三个任务（复用既有的 MSST 扫描与 `choose_stem_for_task`），
用户可在设置里改。输出轨仍取自模型自己声明的 `training.instruments`，不猜。

**分块进度**：MSST 没有进度回调，只把 `tqdm` 打到终端。桥接进程在**自己进程内**把
`utils.utils.tqdm` 换成一个上报用的替身（不改 MSST 代码），从中取出 `n / total`。
该 tqdm 的 `total` 是**样本数**，而任务面板显示的是秒，因此按模型配置的 `sample_rate`
换算后再上报，与 PyMSS 的进度同单位、共用同一个确定进度条。

两个实现细节：上报按「变化 ≥0.5% 或间隔 ≥0.3 秒」节流，否则每个分块一条会刷屏；
