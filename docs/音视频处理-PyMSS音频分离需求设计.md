...
- 支持老 MSST-WebUI 用户只安装 PyMSS 托管运行时，并原地复用已经下载的兼容模型，
  不要求复制或重新下载大体积权重。
- 面向普通用户只提供“分离人声”“分离伴奏”“提取和声”三类任务，不暴露完整模型
  列表和底层推理参数。
...
- 不提供面向普通用户的任意模型市场、手工填写底层推理参数、合奏模式、训练、验证、
  音频转 MIDI 等高级功能；旧 MSST 模型导入仅用于三类固定任务的兼容模型映射。
...
- 三个任务入口：分离人声、分离伴奏、提取和声。
- 当前任务卡：阶段、耗时、粗粒度进度、取消/停止入口、错误信息。
- 结果卡：输出文件列表、试听、打开文件、打开目录。
- 设置入口：安装位置、运行时来源、服务地址、下载源、设备诊断和修复安装。
...
3. **任务选择区**：三张等宽任务卡，分别为“分离人声”“分离伴奏”“提取和声”。每张
...
├── models/                    # 按预设下载的模型与配置
├── manifests/
│   ├── runtime-manifest.json  # 工作台托管运行时清单
...
该模式仍使用工作台下载和维护的 PyMSS 托管运行时，但不强制下载推荐模型。用户选择一个
...
仍然有效的运行时；应先落到 `staging/`，校验成功后再切换。
...
| 首次进入音频分离 Tab | 完整运行时清单检查；检查所需模型的存在状态 |
| 启动服务前 | 完整运行时检查 + 可执行探测 + 版本兼容性 |
| 提交任务前 | 当前任务预设依赖的模型、配置和辅助文件检查 |
| 下载/修复完成后 | 大小、SHA-256、版本和启动冒烟检查 |

清单完整校验可缓存，但文件时间、大小或目录状态变化后必须失效。若用户在程序运行期间
删除文件，服务进程退出、任务失败或下一次提交任务时也应重新触发校验。
...
| 运行时不完整 | `python.exe`、PyMSS 包或 DLL 缺失 | 安装损坏，需要修复 |
...
模型缺失时不强制用户修复整个运行时。用户点击任务后，可在确认预计下载量后只下载该
任务需要的模型。
...
| `RUNTIME_DOWNLOADING` | 正在下载托管运行时 | 查看进度、取消 |
| `RUNTIME_VERIFYING` | 正在校验并切换运行时 | 等待，不允许启动服务 |
| `INSTALL_MISSING` | 保存的托管目录已不存在 | 修复、重新定位、移除配置 |
| `INSTALL_DAMAGED` | 托管运行时文件不完整或损坏 | 修复、重新完整安装 |
| `VERSION_INCOMPATIBLE` | 托管 PyMSS 版本不在当前工作台兼容范围 | 确认下载量后原地更新运行时 |
| `EXTERNAL_VERSION_INCOMPATIBLE` | 用户选择的外部 PyMSS 版本不兼容 | 用户自行升级后重新检测，或重新选择环境 |
| `INSTALLED_STOPPED` | 运行时有效，服务未启动 | 启动服务、检查更新、修复 |
...
| 分离伴奏 | 原曲音频 | 伴奏 | 使用伴奏提取预设，保存 `instrumental` |
| 提取和声 | 原曲音频 | 主唱、和声 | 固定两阶段链路，输出主唱与纯和声语义 |

托管默认链路先用 Karaoke 模型从原曲保留主唱并得到“伴奏 + 和声”残差，再用
`inst_v1e` 从残差中提取和声；不得把第一阶段残差直接误标为纯和声。用户映射外部
Karaoke 模型时，链路先获得完整人声，再由外部模型拆分主唱与和声。

两阶段任务会缓存第一阶段的完整结果 ZIP。缓存键包含输入内容 SHA-256、预设 ID/版本、
...
|---|---|---:|---|
| 分离人声 | `big_beta5e` | 1,479,749,810 bytes | `vocals → 人声` |
| 分离伴奏 | `inst_v1e` | 913,102,724 bytes | `instrumental → 伴奏` |
| 提取和声第 1 阶段 | `mel_band_roformer_karaoke_becruily` | 1,719,139,254 bytes | `vocals → 主唱`，保留 residual |
| 提取和声第 2 阶段 | `inst_v1e` | 913,102,724 bytes | residual 的 `vocals → 和声` |
...
且 HTTP 输入需要由客户端解码、重采样并传输完整 PCM。首版不得伪造精确百分比，应显示
真实可知的阶段：
...
- 自动下载运行时和模型必须验证大小与 SHA-256；下载源和最终 URL写入日志。
...
- 已用真实 CPU 托管运行时启动 server，并通过 `/health`、API v1 能力与进程停止冒烟。
...
- 删除一个运行时关键文件后，能显示“安装损坏”，修复后服务可启动。
- 下载中途强制退出后，不会把半成品识别为有效安装。
- 删除一个模型后，运行时仍保持有效；只在对应任务上提示并补下该模型。
...
3. **预设**：采用 §8.3 的三个模型和固定两阶段和声链路；首版不提供手动模型参数。
...
8. **运行时资产**：独立公开资产仓库 `karaoke-studio/karaoke-studio-runtime` 使用 GitHub
...
用户升级工作台后，客户端会比较已安装清单中的 PyMSS 版本、运行时修订和 Python ABI。
任一不匹配均进入“版本不兼容”状态，显示待下载量并等待用户确认；确认后只原子替换
`runtime/` 和运行时清单，保留模型、MSST 映射、中间缓存与日志，失败则回滚旧运行时。
用户指定的外部 PyMSS 环境不由工作台写入或升级，只提示用户自行升级后重新检测，或改选
其他环境。升级不静默开始，也不把新版底座、torch 或模型加入工作台安装包。

安装、修复和升级在替换运行时前会先停止工作台拥有的服务；新目录切换后，在旧目录备份
...
2. **状态区形态**：合并 §3.3 服务状态卡与 §3.4 状态与操作条为**一条顶部状态条**：
   ...
   折叠区。
3. **设置入口**：独立设置对话框，仿全局设置（Pivot 分页：安装与运行时 / 服务与
...
2. 当前任务区用六阶段步骤条：准备音频 → 下载模型 → 加载模型 → 分离处理中 →
   编码/接收输出 → 保存结果；当前阶段高亮，下载阶段显示真实字节进度条，其余阶段
   用不确定进度条 + 已用时间；严格不伪造百分比（§9.3）。「提取和声」两阶段作为
   该任务的子阶段展示。
...
└── separation/
    ...
    ├── runtime.py / service.py  ✅ 托管运行时与服务进程生命周期
...
1. ✅ **`separation/settings_dialog.py`**：`SeparationSettingsDialog(QDialog)`，Pivot +
   ...
   不 import gui_qt）：
   - 安装与运行时：安装位置（只读 + 更改…/打开目录）、运行方式、PyMSS 版本 + 重新检测；
   - 服务与下载：下载源 ComboBox（ModelScope / Hugging Face / HF Mirror）、经能力检测
     向导重新配置外部服务、输出目录显示与更改；
   - 诊断与日志：设备、版本、运行时、模型和错误诊断，打开日志目录、只读日志预览
...
1. **`MockSeparationBackend` 不再假设「有安装目录 ⇒ 模型齐全」**。原实现里
   ...
   §8.4「安装托管运行时时不默认下载全部模型」直接冲突，也让「需下载 x.xx GB」分支
...
结构性冗余：多个步骤的 `hint` 在复述 `title`（「选择安装位置」+「PyMSS 运行时会安装到
...
- 首次进入音频分离页异步执行完整清单哈希；启动服务、提交任务和健康检查再次按对应层级
  验证。运行时、推荐模型和外部 MSST 权重分别恢复，不相互触发整包重装。
...
| 项目 | 当前证据 | 状态 |
|---|---|---|
| Python 编译 | `compileall` 覆盖音频处理包与运行时构建器 | 通过 |
| 状态机/UI/运行时/MSST/API/缓存 | `tests/test_audio_processing_*.py` 与 `tests/test_build_pymss_runtime.py` | 87 项定向测试通过 |
...
| 远端托管运行时资产 | 独立仓库 `karaoke-studio/karaoke-studio-runtime` 的 `pymss-runtime-v2.0.18-r1` | 已发布；CPU/CUDA 清单与分卷公网回读通过 |
