...
| 首次进入音频分离 Tab | 完整 Runtime 清单检查；检查所需模型的存在状态 |
| 启动服务前 | 完整 Runtime 检查 + 可执行探测 + 版本兼容性 |
...
| `RUNTIME_VERIFYING` | 正在校验并切换 Runtime | 等待，不允许启动服务 |
...
- 首次进入音频分离页异步执行完整清单哈希；启动服务、提交任务和健康检查再次按对应层级
  验证。Runtime、推荐模型和外部 MSST 权重分别恢复，不相互触发整包重装。
