...
| 动态字符越界横线修复 | N3 的 before/after 裁剪纵向始终覆盖完整画面且每帧读回完整 surface；工作台此前使用原行框纵向裁剪，且带状读回只为旋转翻转扩展动画范围，遗漏 Utopia/逐字垂落 | 横排走字裁剪改为全画面纵向范围；带状读回按变换后字形、描边、发光和阴影联合包围盒扩展。三种动态特效整帧/带状读回逐字节一致，Direct2D GPU 完整回归 `182 passed, 3 skipped`；《メフィスト》63～64.5 秒 1080p60、描边 15、发光、3 worker 稳态 Utopia 96.29 fps、逐字垂落 101.16 fps |
...
10. ✅ 对齐 N3 的全画面纵向走字裁剪语义，并按动画后视觉包围盒扩展带状读回，修复 Utopia、旋转翻转和逐字垂落越出原行框时的横向截断线。
