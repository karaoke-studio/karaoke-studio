...
- C5 预览初始接线已落地，但 2026-06-27 起 native preview 改为实验 opt-in；默认仍使用 Python
  `AsyncSubtitleRenderer`。显式设置 `KROK_SUBTITLE_NATIVE_RENDER=1` 且 sidecar 可用时，graphics preview 才会使用
  native `render_range` + shared memory 帧；generation / 当前时间会过滤过期帧，native 失败时回退后台 Python renderer。
...
- `KROK_SUBTITLE_NATIVE_RENDER=1`：启用实验 native preview；未设置时默认走 Python async preview。
...
- `frontend/preview_async.py` 新增 opt-in `NativeAsyncSubtitleRenderer`。当显式设置
  `KROK_SUBTITLE_NATIVE_RENDER=1` 且能解析到 sidecar exe 时，预览可通过 native `render_range` 读取
  shared-memory `frame_ready` 并向 UI 发 `QImage`。
- `frontend/preview_graphics.py` 会在 async preview 开启且 native opt-in 可用时选择 native renderer，否则继续使用原 Python
...
- C++ 侧不能只做 sidecar 多线程调度；需要对齐 Python 侧已经成熟的 bake/layer 缓存策略。
- 不要恢复整帧预览缓存作为主方案。`字幕渲染-预渲染帧缓存方案评估.md` 已记录过：`utopia` 每帧有连续抖动、缩放、
  扫光，按时间桶复用整帧会产生时间吸附、重影和错位；它对首次连续播放也没有收益。
- 正确方向是迁移 Python `engine/layers.py` / `engine/painter.py` 里的 layer bake 思路：静态文字、ruby、glow、
  path stack 等可复用部分缓存，动态动画仍按帧组合。

本地已落地的改动（尚未提交时请先看 worktree diff）：

- `frontend/preview_async.py`：native preview 默认 look-ahead 调整为 6，ring slot 至少为 look-ahead + 2；
  一个 native range 内复用同一个 `SharedFrameRingReader`，避免每帧 attach/detach shared memory。
- `native/subtitle_renderer/src/main.cpp`：新增 native text layer cache、thread-local layout cache、ruby 静态层缓存、
  glyph-run 级 before/after body 缓存，并让 inline role 的主歌词也走 glyph-run 缓存路径。
- 保留 `KROK_SUBTITLE_NATIVE_TEXT_LAYER_CACHE=off/0/false/no` 回退开关；排查画质或内存问题时先用它二分。
- 测试已覆盖：
  - native preview 默认前瞻与 range 内 reader 复用；
  - 主歌词/ruby text layer cache 重复命中；
  - inline role glyph-run cache 重复命中；
  - glow cache 测试已显式关闭 text layer cache，避免统计互相污染。

当前验证基线：

```powershell
powershell -ExecutionPolicy Bypass -File scripts\run_native_renderer_smoke.ps1
C:\Python314\python.exe -m pytest tests\test_subtitle_render_native_protocol.py tests\test_subtitle_render_transport.py tests\test_subtitle_render_native_benchmark.py -q
```

本机结果：native smoke 通过；pytest 为 `128 passed`。

`A stain` 33s preview probe：

```powershell
C:\Python314\python.exe scripts\probe_native_preview_stats.py --lrc "D:\カラオケ\songs\A stain\A stain.lrc" --video "D:\カラオケ\songs\A stain\A stain.mp4" --duration-ms 33000 --fps 60 --width 1920 --height 1080 --offscreen --out .bench\native_preview_a_stain_33s_glyphrun.csv
```

当前 summary：

```text
requests=1943 elapsed_ms=33031 last_t_ms=33000 cache_hits=1867 cache_misses=79
cache_hit_rate=0.9594 future_frames_cached=2464 stale_frames_dropped=0
generations_cancelled=1 native_generation_cancelled_events=0 range_done_events=696
native_renderer_failures=0
```

`A stain` benchmark 当前基线：

```text
scenario          : A stain
cache / native    : on / range:4
frames / size     : 648 1920x1080
python mean / p95 : 2.6176 ms / 3.8257 ms
native roundtrip  : 1.0193 ms / 1.0193 ms
native render     : 3.4854 ms / 4.2748 ms
speedup           : 2.57x (render-only 0.75x)
```

这里要注意：wall-clock 已经靠多线程与前瞻变快，但单帧 native render-only 仍慢于 Python，说明缓存 parity 还没补完。

下一轮优先级：

1. 拆 after-glow layer parity。
   Python 有 `_GlyphRunAfterGlowLayer`，after 部分的 glow 是独立层/独立 key；native 目前仍偏向把部分文字 stack 整体 bake。
   下一步应把 after-glow 单独缓存，减少重复 bake，并对齐 glow-before/after 在 clip 下的语义。
2. 审计 `utopia` / ruby group 的 transformed glow。
   native 已有 `paintTransformedTextStackWithFills` 与 cached transformed glow 路径，但需要按 Python `_RUN_GLOW_CACHE`
   语义核对主歌词、ruby、group 的 source path 是否一致。body 仍应保持矢量绘制，避免把动态 `utopia` body 缓成变换位图导致发糊或错位。
3. 把 C++ 分散 helper cache 收敛成 LayerCompositor-like 合同。
   建议抽出 native layer 概念：`static_key` / `bake` / `animate`。不要继续为每个特效堆独立的 ad-hoc cache，
   否则后续 title、signal、vertical 会越来越难维护。
4. 降低 glyph-run key/layout string churn。
   现在 non-inline 已有快速路径，inline role 也能缓存；但 role-heavy 行仍会每帧构造 run/key。可考虑把 run partition
   或 layout signature 放进 layout cache，减少字符串和 vector 重建。
5. 补静止保持段的 identical-frame short-circuit。
   仅用于非动画/静止 hold 段；不要覆盖扫光、入退场、`utopia` 动态帧。对应思路见
   `字幕渲染-预渲染帧缓存方案评估.md` 的 §8.3a。
6. 若 native 后续扩展 vertical/title/signal，按 Python bake 方式迁移。
   vertical 对应 `_BakedPathStackLayer`；title overlay 适合 baked path/text layer；signal lits 保持动态层，
   只在 profiling 证明有收益时缓存静态 shape atlas。
7. 补诊断输出。
   `bench_native_renderer.py` 目前主要看 glow cache；后续应输出 `text_layer_cache_*`、`layout_cache_*`、
   eviction、cache bytes 或近似内存占用，便于和 Python 侧缓存命中率对齐。
8. 调整内存/eviction 策略。
   当前 text layer cache 上限是固定值；用多首真实工程压测后，再决定是否加 env override，例如最大项数或最大字节数。

禁区：

- 不要把 `utopia` 动态帧重新接回整帧预渲染缓存。
- 不要把 `utopia` body 直接缓存为已变换 bitmap 后逐帧缩放/旋转；除非先按
  `字幕渲染-管线优化调研.md` §9 设计量子化重栅格方案并补像素 diff 测试。
- 不要为了命中率放宽 cache key 到会跨 style、font、DPR、stroke/glow 参数复用的程度；字幕错位/跳字通常比慢更难排查。
