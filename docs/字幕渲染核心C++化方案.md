...
- C5 预览初始接线已落地：sidecar 可用时，graphics preview 默认使用 native
...
- `frontend/preview_async.py` 新增 default-on `NativeAsyncSubtitleRenderer`。当未设置
  `KROK_SUBTITLE_NATIVE_RENDER=0` 且能解析到 sidecar exe 时，预览可通过 native `render_range` 读取
