...
- 产品协议雏形已落地：`render_range` / `cancel_generation` 支持 generation、取消、后台 worker、逐帧 `frame_ready`；
  `frame_ready` 已携带 `QSharedMemory` ring slot metadata，native 会写入 RGBA8888 payload。Python wrapper 目前只接收
  slot metadata，尚未 attach shared memory 读取像素。

**下一步**：实现 Python 侧 shared memory slot consumer，把 `frame_ready` 的 slot metadata 转成可校验的 `QImage`/bytes，
再接 C5 预览调度。
